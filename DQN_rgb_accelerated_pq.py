...
def createNetwork():
    ...
    #W_fc1 = weight_variable([11776, 100])
...
def trainNetwork(s, readout, h_fc1, sess):
    ...
    checkpoint = tf.train.get_checkpoint_state("saved_networks_pq")
    ...
    while True:
        ...
        if t % FRAME_PER_ACTION == 0:
            ...
            #    a_t[random.randrange(ACTIONS)] = 1
            #    selector = "RND"
            if t <= OBSERVE + IMITATE:
                action_index = im_action
                a_t[im_action] = 1
                selector = "IM"
            elif random_select <= epsilon:
                action_index = random.randrange(ACTIONS)
                a_t[random.randrange(ACTIONS)] = 1
                selector = "RND"
            else:
        ...
        #    saver.save(sess, 'saved_networks_pq/' 'checkpoint', global_step = t)
        #    break
        #    break
        if  t > OBSERVE + IMITATE:#t > OBSERVE: #and t % 100 == 0: #True:#t > OBSERVE: #and
...
def telemetry():
    while True:
        ...
        im_action = im_replay_01.get_im_response()
